import {
  WIKI_API_URL,
  GOOGLE_SHEET_URL,
} from "../../../leagueConfiguration/BigBrother_26";
...
export default async function Scoring() {
  const dataFetcher = getWikipediaContestantDataFetcher(
    WIKI_API_URL,
    "HouseGuests",
  );

  const listOfContestantRoundLists = await generateListOfContestantRoundLists(
    dataFetcher,
  ...
  return (
    <div>
      ...
      <br />
      <ContestantSelector
        listOfContestantRoundLists={listOfContestantRoundLists}
      />
      <p>
        This season's contestant data has been sourced from{" "}
        <a className="standard-link" href={GOOGLE_SHEET_URL}>
          this google sheet
        </a>{" "}
        which was populated using a google form.
      </p>
      <br />
    </div>
  );
