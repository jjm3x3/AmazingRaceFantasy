...
export default function Navigation({ pages }: { pages: IPage[] }) {
  ...
  return (
    <nav id={styles["navigation"]} data-testid="navigation">
      <NavigationItem
        ...
        hasSubpages={pages.length > 0}
        children={pages.map((page: IPage) => {
          const keyName = page.name.toLowerCase().replaceAll(" ", "-");
          const hasSubpages =
            page.hasOwnProperty("subpages") && page.subpages.length > 0;
          ...
          return (
            <li
              key={`nav-toplevellink-${keyName}`}
              className={styles["nav-toplevel-page"]}
            >
              <NavigationItem
                ...
                children={page.subpages.map((subpage: ISubpage) => {
                  const subpageKeyName = subpage.name
                    .toLowerCase()
                    .replaceAll(" ", "-");
                  return (
                    <li
                      key={`nav-toplevellink-${keyName}-sublink-${subpageKeyName}`}
                    >
                      <Link
                        href={subpage.path}
                        className={styles["sub-level-link"]}
                      >
                        {subpage.name}
                      </Link>
                    </li>
