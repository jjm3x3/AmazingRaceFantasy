...
export default function NavigationItem({ inputAttr, labelAttr, listAttr, children, hasSubpages }: INavigationItem) {
    ...
    return (<>
        <input id={inputAttr.id} className={inputAttr.classes} type="checkbox" onChange={(e)=> setIsHidden(!e.target.value)}/>
        <label htmlFor={inputAttr.id} id={labelAttr.id} aria-controls={labelAttr.aria.controls} data-testid={labelAttr.testId} className={labelAttr.classes }>{labelAttr.content}</label>
