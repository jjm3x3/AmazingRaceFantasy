...
export function isPartialContestantData(
  contestantRowData: ITableRowData,
): boolean {
  ...
  return (
    (contestantRowData.name == null || contestantRowData.name === "") &&
    (!contestantRowData.name2 || !contestantRowData.col2)
  );
}
...
export function getCompetingEntityList(contestantData: ITableRowData[]): any {
  ...
  contestantData.forEach((element, index) => {
    ...
    } else if (status.toLowerCase().includes("runner-upd")) {
      // added the d to distinguish from amazing-race
      isParticipating = false;
  ...
  const contestantsWithEliminationOrder = sortedContestants.map(
    function (contestant) {
  ...
  const contestantsSortedByEliminationOrder =
    contestantsWithEliminationOrder.sort(function (a, b) {
