...
export function getWikipediaContestantDataFetcher(
  wikiUrl: string,
  contestantSectionName: string,
): () => Promise<ITableRowData[]> {
...
function findSectionIndexByAnchor(
  sections: Section[],
  anchor: string,
): number | undefined {
...
export async function getWikipediaContestantData(
  wikiUrl: string,
  contestantSectionName: string,
): Promise<ITableRowData[]> {
  ...
  const sectionIndex = findSectionIndexByAnchor(
    sections,
    contestantSectionName,
  );
  ...
  const contestantData = cheerioFilter
    .map((index, element) => {
      const $row = $(element);
