...
async function generateContestantRoundScores(
  ...
  listOfContestantLeagueData.map((contestant) => {
    ...
    result.addContestantRoundScores(
...
export default async function generateContestantRoundScoreComponent(
  dataFetcher: () => Promise<ITableRowData[]>,
  listOfContestantLeagueData: any[],
) {
  const contestantScores = await generateContestantRoundScores(
    dataFetcher,
    listOfContestantLeagueData,
  );
