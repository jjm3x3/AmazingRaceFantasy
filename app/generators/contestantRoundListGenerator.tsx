...
export default async function generateListOfContestantRoundLists(
  ...
  const teamDictionary = pageData.props.runners.reduce(
    (acc: Dictionary<ITeam>, t: ITeam) => {
  ...
  const roundScores: IRound[] = LeagueStanding.generateContestantRoundScores(
    reverseTeamsList,
    numberOfRounds,
    "*perfect*",
    perfectScoreHandicap,
  );

  return listOfContestantLeagueData.map((contestant) => {
    const currentSelectedContestantTeamsList = contestant.ranking.map(
      (x: string) => {
    ...
    const contestantRoundScores: IRound[] =
      LeagueStanding.generateContestantRoundScores(
        currentSelectedContestantTeamsList,
        numberOfRounds,
        contestant.name,
        contestant.handicap,
      );
...
export function getNumberOfRounds(teams: ITeam[]): number {
  return teams.reduce((acc: number, x: ITeam) => {
    return x.eliminationOrder > acc && x.eliminationOrder !== Number.MAX_VALUE
      ? x.eliminationOrder
      : acc;
  }, 0);
}
