    ...
    <div>
      ...
      <main>
        <p className="site-notice">
          ...
          participating, jump in{" "}
          <Link className="standard-link" href={noticeLink}>
            here
          </Link>
          . If you would like to participate in this league or future leagues
          please email inquiry to{" "}
          <Link
            className="standard-link"
            href="mailto:xfactorleaguesite@gmail.com"
          >
            xfactorleaguesite@gmail.com
          </Link>
          .
        </p>
        {pages.map((p: IPage) => {
              ...
              <div className="md:flex md:flex-row">
                {p.subpages.map((pSub: ISubpage) => (
                  <p className="standard-link-container md:basis-1/3">
                    <Link
                      className="standard-link"
                      key={`links-section-${keyName}-link-${pSub.name.toLowerCase().replaceAll(" ", "-")}`}
                      href={pSub.path}
                    >
                      {pSub.name}
                    </Link>
